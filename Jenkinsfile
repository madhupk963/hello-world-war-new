    ...
    stages {
        ...
        agent {label 'publish'} 
            steps { 
               ...
               sh 'helm repo update'
               sh 'helm repo list'
               sh 'helm upgrade --install tomcat helm/tomcat --version=${BUILD_VERSION} --set selector_matchlabels=tomcat --set deployment_name=tomcat --set replicas=2 --set docker_reg_name=madhusudhanpk --set docker_repo_name=tomcat --set image_tag=${BUILD_VERSION} --set port_name=tomcat --set target_port=8080 --set port=8080 --set favorite.drink=coffee --set favorite.food=pizza'
            }
        }    
    } 
}
